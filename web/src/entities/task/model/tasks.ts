...
export const normalizeTask = (data: Task) => normalize<Task, { tasks: NormalizedTasks }>(data, taskSchema)
export const normalizeTasks = (data: Task[]) => normalize<Task, { tasks: NormalizedTasks }>(data, [taskSchema])
...
export const getTasksListAsync = (params?: typicodeApi.tasks.GetTasksListParams) => (dispatch: Dispatch) =>
  useQuery<AxiosResponse<Task[]>>(TASK_LIST_QUERY_KEY, () => typicodeApi.tasks.getTasksList(params), {
    onSuccess: ({ data }) => dispatch(taskModel.actions.setTasksList(data)),
    refetchOnWindowFocus: false,
  })

export const getTaskByIdAsync = (params: typicodeApi.tasks.GetTaskByIdParams) => (dispatch: Dispatch) =>
  useQuery<AxiosResponse<Task>>('task-single', () => typicodeApi.tasks.getTaskById(params), {
...
export const getFilteredTasks = () =>
  useSelector(
    createSelector(
      ...
      (queryConfig: RootState['tasks']['queryConfig'], tasks: RootState['tasks']['data']) =>
        Object.values(tasks).filter(
          (task) => queryConfig?.completed === undefined || task?.completed === queryConfig.completed
        )
    )
  )
...
export const isTaskListLoading = (): boolean => useIsFetching([TASK_LIST_QUERY_KEY]) > 0
